...
public class PostService {
    ...
    public PostDetailDto findById(Long id){
        ...
        PostDetailDto post =  postMapper.findPostWithUserFiles(id);
        if(post == null){
    ...
    public void addLike(Long id){
        ...
        postMapper.updatePostLikes(post.getId());
    }
    @Transactional
    public void updatePost(Long id,PostDto postDto,String author,List<MultipartFile> files) throws IOException {
        ...
        savePost.setContent(postDto.getContent());
        //나머지 좋아요는 엔티티를 별도로 수정하는것이 아니기에 기존 값이 적용
        //> 이유는 상세 페이지가 엔티티와 연동되어있기 때문
        //유저 네임 및 유저도 다시 넘길 필요 X 변동이 없기때문(유저도 엔티티로 연동됨)

        postMapper.updatePost(savePost);


        List<FileDto> filDtos = fileMapper.findFilesByPostId(id);

        if(files == null || files.isEmpty() || files.get(0).isEmpty()){
            return;
        }

        for(FileDto fileDto : filDtos) {
        ...
        for(MultipartFile file : files){
            ...
            newfileEntity.setPostId(savePost.getId());
