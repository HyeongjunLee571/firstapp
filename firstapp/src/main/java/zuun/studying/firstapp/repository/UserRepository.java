...
public interface UserRepository extends JpaRepository<User, Long>{
    ...
    @Query(value = "INSERT INTO users(username,password,email,userRole,created_at)"+//컬럼명
            "VALUES (:username,:password,:email,:userRole,:createdAt)",nativeQuery = true)//파라미터 명
    void insertUser(@Param("username") String username,
                    @Param("password") String password,
                    @Param("email") String email,
                    @Param("userRole") String userRole,
                    @Param("createdAt")LocalDateTime createdAt);//값 전달(새로운 값 수정 및 추가 시 새로운 값을 컬럼과 파라미터와 값에도 추가해야함)
